...
class AimlHandler(ContentHandler):
    ...
    def _pushWhitespaceBehavior(self, attr):
        ...
        The string's value is taken from the "xml:space" attribute, if it
        exists and has a legal value ("default" or "preserve").  Otherwise,
        the previous stack element is duplicated.

        """
        assert len(self._whitespaceBehaviorStack) > 0,\
            "Whitespace behavior stack should never be empty!"
        try:
            if attr["xml:space"] == "default"\
                    or attr["xml:space"] == "preserve":
    ...
    def _startElement(self, name, attr):
        if name == "aiml":
            ...
            if (self._state != self._STATE_InsideAiml) or self._insideTopic:
                raise AimlParserError(
                    "Unexpected <topic> tag", self._location())
            try:
        ...
        elif name == "template" and (not self._insideLearn):
            # <template> tags are only legal in the AfterPattern and AfterThat
            # states
            if self._state not in [self._STATE_AfterPattern,
                                   self._STATE_AfterThat]:
                raise AimlParserError(
                    "Unexpected <template> tag " + self._location())
            ...
            # Turn-on insideLearn flag
            if name == "learn":
                if not self._insideLearn:
                    self._insideLearn = True
                else:
                    raise AimlParserError(
                        ("Unexpected <%s> tag " % name) + self._location())
        else:
        ...
        if self._state == self._STATE_InsidePattern:
            ...
            if textElemOnStack:
                ...
                self._elemStack[-1].append(
                    ["text",
                     {"xml:space": self._whitespaceBehaviorStack[-1]}, text])
        else:
    ...
    def endElement(self, name):
        ...
        if self._currentUnknown != "":
            ...
            if name == self._currentUnknown:
                self._currentUnknown = ""
            return
        if self._skipCurrentCategory and (not self._insideLearn):
    ...
    def _endElement(self, name):
        ...
        if name == "aiml":
            ...
            if self._state != self._STATE_InsideAiml:
                raise AimlParserError(
                    "Unexpected </aiml> tag " + self._location())
            self._state = self._STATE_OutsideAiml
            ...
            # Store the current pattern/that/topic and
            ...
            if elem[0] == "condition":
                ...
                # Turn-off insideLearn flag
            if elem[0] == "learn" and self._insideLearn:
                self._insideLearn = False
        else:
    ...
    def _validateElemStart(self, name, attr, version):
        ...
        for a in attr:
            if a in required:
                continue
            if a[0:4] == "xml:":
                # attributes in the "xml" namespace can appear anywhere
                continue
        ...
        if not canBeParent:
            raise AimlParserError(
                ("<%s> elements cannot have any contents " % parent)
                + self._location())
        ...
        # present in the <condition> parent.
            ...
            if not (parent == "random" \
                        or parent == "html:ul" or nonBlockStyleCondition):
                raise AimlParserError(
                    ("Unexpected <li> element "
                     + "contained by <%s> element " % parent)
                    + self._location())

...
class AimlHandler(ContentHandler):
	...
	def _startElement(self, name, attr):
		...
		elif name == "that" and self._state == self._STATE_AfterPattern and (not self._insideLearn):
	...
	def _characters(self, ch):
		...
		if self._state == self._STATE_InsidePattern:
			...
			try:
				...
				if not canBeParent:
					raise AimlParserError, ("Unexpected text inside <%s> element (not canBeParent)"%parent)+self._location()
					...
					if len(text.strip()) == 0:
						...
						raise AimlParserError, ("Unexpected text inside <%s> element (non-whitespace text)"%parent)+self._location()
			except IndexError:
	...
	def _endElement(self, name):
		...
		elif name == "that" and self._state == self._STATE_InsideThat and (not self._insideLearn):
	...
	_validationInfo101_ext = {
		"bot":      	( ["name"], [], False ),
		"category":     ( [], [], True ), # can be inside <learn> (non-standard aiml)
		...
		"li":           ( [], ["name", "value"], True ),
		"lowercase":    ( [], [], True ),
		"pattern":      ( [], [], True ), # can be inside <learn> (non-standard aiml)
		"person":       ( [], [], True ),
		"person2":      ( [], [], True ),
